...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`bg-gray-800 text-white w-64 h-full px-4 py-8 fixed left-0 z-40 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
          ...
          {menuData.map((item, index) => (
            <li key={index} className="mb-2">
              {item.subMenu.length > 0 ? (
                  ...
                  {openMenu === index && (
                    <ul className="pl-4 mt-2">
                      {item.subMenu.map((subItem, subIndex) => (
                        <li key={subIndex}>
                          <Link href={subItem.path} className="block py-2 px-4 rounded hover:bg-gray-600">
                            {subItem.title}
                          </Link>
                        </li>
                      ))}
                ...
                <Link href={item.path} className="block py-2 bg-[#2c907f] px-4 rounded hover:bg-[#269885]">
                  {item.title}
                </Link>
              )}
            </li>
